...
def process_video_to_array(input_path, logger, target_frame_count=48, target_resolution=(480, 640)):
    # 打开输入视频
    ...
    while video.isOpened():
        ...
        # 只处理需要的帧
        if current_frame >= processed_frame * frame_interval and processed_frame < actual_target_frames:
            # 调整分辨率
    ...
    logger.info(f"处理完成！输出数组形状 - 类型 - 最大值 - 最小值 - 平均值： {video_array.shape,video_array.dtype,video_array.max(),video_array.min(),video_array.mean()}")
    return video_array
